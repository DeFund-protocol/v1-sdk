import ethers$1, { Contract, utils, BigNumber, ethers, constants } from 'ethers';
import { FeeAmount, TickMath, Pool, Position, NonfungiblePositionManager, computePoolAddress, FACTORY_ADDRESS, nearestUsableTick, priceToClosestTick, TICK_SPACINGS } from '@uniswap/v3-sdk';
...
import { Token, Percent, CurrencyAmount, Ether, Price } from '@uniswap/sdk-core';
 ...
 * helper methods
 */
const getTargetAddressFromOpType = (chainId, opType) => {
    switch (opType) {
        case 'swap':
...
const addLiquidityCalldata = async (chainId, signer, params, fundAddress) => {
    ...
    const { calldata } = NonfungiblePositionManager.addCallParameters(positionToMint, options);
    return calldata;
};
...
class Fund {
    ...
    fallbackPath(token0, token1) {
        ...
        if (isEqualAddress(token0, weth) || isEqualAddress(weth, token1)) {
            return encodePath$1([token0, token1], [FeeAmount.MEDIUM]);
        }
        else {
            return encodePath$1([token0, weth, token1], [FeeAmount.MEDIUM, FeeAmount.MEDIUM]);
        }
    }
    async getSellParams(sellRatio) {
        ...
        console.debug('=> target amount', ethers$1.utils.formatUnits(targetAmount, decimals), `(${this.mergePercentage}%)`);
        ...
        if (underlyingTokenBalance.balance.gte(targetAmount)) {
            return {
                sellType: ethers$1.constants.Zero,
                minPriority: ethers$1.constants.MaxUint256,
        ...
        const params = {
            sellType: ethers$1.constants.Zero,
            minPriority: ethers$1.constants.Zero,
        ...
        let totalAssetAmount = ethers$1.constants.Zero;
        ...
        for (const group of groupedAssets) {
            ...
            for (const asset of group) {
                if (asset.token && asset.balance.eq(ethers$1.constants.Zero))
                    continue;
                if (asset.tokenId &&
                    asset.amount0.add(asset.fee0).eq(ethers$1.constants.Zero) &&
                    asset.amount1.add(asset.fee1).eq(ethers$1.constants.Zero))
                    continue;
                if (asset.token) {
            ...
            if (params.minPriority.gt(ethers$1.constants.Zero))
                break;
        }
        const paths = [];
        for (const token of fund.allowedTokens) {
            const balance = balances[token] || ethers$1.constants.Zero;
            if (balance.eq(ethers$1.constants.Zero) ||
...
class UniversalSDK {
    ...
    async executeApproveToken(maker, fundAddress, params, overrides) {
        return await this.fund(fundAddress).executeApproveToken(params, maker, fundAddress, overrides);
    }
