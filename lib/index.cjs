...
class Fund {
    ...
    async getFundInvistors(page = 0, pageSize = 100) {
        ...
        return results.filter((item) => item.lpAddr !== ethers.constants.AddressZero);
    }
    async executeSwap(maker, fundAddress, params, refundGas, overrides) {
    ...
    async executeApproveToken(params, maker, fundAddress, overrides) {
        maker ||= await getAddressFromSigner(this.signer);
        const needApprove = tokenNeedApprove(this.signer, params.token, fundAddress, this.fundProxyAddress, params.amount || ethers.constants.MaxInt256);
        if (!needApprove)
            return;
        const approveParams = [this.fundProxyAddress, params.amount || ethers.constants.MaxInt256];
