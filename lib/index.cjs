...
class UniversalSDK {
    ...
    async executeApproveToken(maker, fundAddress, params, overrides) {
        const needApprove = tokenNeedApprove(this.signer, params.token, fundAddress, fundAddress, params.amount || ethers.constants.Zero);
        if (!needApprove)
            return;
        const approveParams = [fundAddress, params.amount || ethers.constants.MaxInt256];
