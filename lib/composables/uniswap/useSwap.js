...
export class UniswapSwap {
    ...
    async exactInputCalldata(params, recipient) {
        const path = await exactInputPath(params.tokenIn, params.tokenOut, params.amountIn, this.chainId, this.signer);
        const swapParams = {
    ...
    async exactOutPutCalldata(params, recipient) {
        const path = await exactOutputPath(params.tokenIn, params.tokenOut, params.amountOut, this.chainId, this.signer);
        const swapParams = {
            recipient: this.calcRecipient(params, recipient),
            path: path.path,
        ...
        switch (true) {
            case isEqualAddress(params.tokenIn, this.wethAddress):
                ...
                return this.encodeCalldata(SwapRouter02ABI, 'multicall(uint256,bytes[])', [params.expiration, [output, refund]]);
            case isEqualAddress(params.tokenOut, this.wethAddress):
                const unwrap = this.encodeCalldata(SwapRouter02ABI, 'unwrapWETH9(uint256,address)', [params.amountOut, recipient]);
                return this.encodeCalldata(SwapRouter02ABI, 'multicall(uint256,bytes[])', [params.expiration, [output, unwrap]]);
            default:
                return output;
        }
    }
