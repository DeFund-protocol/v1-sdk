...
import { UniswapSwap } from './uniswap/useSwap';
...
export class FundAssetConvert {
    ...
    async executeAssetsConvert(maker, fundAddress, params, overrides, refundGas = false) {
        ...
        return await SendTransaction(this.fundManagerAddress, FundManagerABI, 'sellAssets', executeParams, overrides, this.signer);
    }
    async executeAssetsConvertWithSlislippage(maker, fundAddress, params, overrides, refundGas = false) {
        const swapParams = {
            opType: 'exactInput',
        ...
        return await new UniswapSwap(this.chainId, this.signer).executeSwap(maker, fundAddress, swapParams, overrides, refundGas);
    }
            ...
            for (let i = 0; i < fundData.allowedTokens.length; i++) {
                if (isEqualAddress(fundData.allowedTokens[i], tokenIn.token)) {
                    const trade = await exactInputPath(params.tokenIn, params.tokenOut, tokenIn.balance
                        ...
                        .div(BigNumber.from(1e4)), this.chainId, this.signer);
