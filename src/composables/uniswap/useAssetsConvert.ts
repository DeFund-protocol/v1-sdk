...
const executeAssetsConvert = async (
  chainId: number,
  signer: Signer,
  maker: string,
  fundAddress: string,
  ...
  const convertParams = await getConvertParams(
    chainId,
    signer,
    fundAddress,
    params
...
const executeAssetsConvertWithSlippage = async (
  chainId: number,
  signer: Signer,
  maker: string,
  fundAddress: string,
  ...
  const tokenIn = assets.tokenBalances.find((item: any) =>
...
const getConvertParams = async (
  ...
  const fundData = await new Fund(chainId, signer, fundAddress).getFunndInfo(
    true
  );
