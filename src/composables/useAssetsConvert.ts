...
export class FundAssetConvert {
  ...
  async getConvertParams(fundAddress: string, params: any) {
    ...
    const targetToken = fundData.tokenBalances.find((item: any) => isEqualAddress(item.token, params.tokenOut));
    ...
    for (const group of groupedAssets) {
      const value = group.reduce((acc: BigNumber, cur: any) => {
      ...
      if (totalAssetAmount.add(value).gte(assetSellAmount)) {
        ...
        if (params.ratio !== 100) params.lastRatio = lastAmount.mul(10000).div(value);
      ...
      if (params.minPriority.gt(constants.Zero)) break;
    }
