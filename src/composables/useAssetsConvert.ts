...
export class FundAssetConvert {
  ...
  async getConvertParams(fundAddress: string, params: any) {
    ...
    const fundData = await useContract(this.fundViewerAddress, FundViewerABI, this.signer).getFundData(
      fundAddress,
      lpAddress,
      true
    );
    ...
    const assets = [
      ...fundData.tokenBalances.filter((item: any) => !isEqualAddress(item.token, fundData.underlyingToken)),
      ...fundData.lpTokens.map((item: any) => ({ ...item, value: item.amountValue0.add(item.amountValue1) })),
    ].sort((a: any, b: any) => b.priority.sub(a.priority));
    const tokenIn = fundData.tokenBalances.find((item: any) => isEqualAddress(item.token, params.tokenIn));
    ...
    if (tokenIn) {
      ...
      for(let i = 0; i < fundData.allowedTokens.length;i++) {
        ...
        if(isEqualAddress(fundData.allowedTokens[i], tokenIn.token)) {
          const trade = await exactInputPath(params.tokenIn, params.tokenOut, tokenIn.balance.mul(BigNumber.from(params.ratio)).div(BigNumber.from(1e4)), this.chainId, this.signer);
    ...
    const needValue = fundData.totalValue.mul(BigNumber.from(params.ratio)).div(BigNumber.from(1e4));
    ...
    for (const asset of assets) {
      ...
      if (asset.token) {
        const amount = asset.balance.mul(params.lastRatio).div(BigNumber.from(1e4));
        balances[asset.token] = balances[asset.token] ? balances[asset.token].add(amount) : amount;
      } else {
        const amount0 = asset.amount0.mul(params.lastRatio).div(BigNumber.from(1e4));
        const amount1 = asset.amount1.mul(params.lastRatio).div(BigNumber.from(1e4));
        balances[asset.token0] = (balances[asset.token0] ? balances[asset.token0].add(amount0) : amount0).add(
          asset.fee0
        );
        balances[asset.token1] = (balances[asset.token1] ? balances[asset.token1].add(amount1) : amount1).add(
          asset.fee1
        );
      }
    }
